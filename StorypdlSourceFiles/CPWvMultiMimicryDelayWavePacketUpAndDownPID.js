// WvMultiMimicryDelayWavePacketUpAndDownPID
...
//            paused
    ...
    if (runtime!=0){
		push();
			translate(130,200);
				showDisplacement(theamplitude*exp(-0.25*pow(thefrequency*runtime,2))*cos(degrees(10*thefrequency*runtime)),0, CIDEABLUE);
			translate(mimicoffset/2, 130);
			showDuration(thedelay, maxDelay);
			translate(-mimicoffset/2, -130);
			if (mimicbutton.buttonisChecked){
				for (i = 1; i < numbermimics; i++) {
					...
					showDisplacement(theamplitude*exp(-0.25*pow(thefrequency*(runtime-i*thedelay),2))*cos(degrees(10*thefrequency*(runtime-i*thedelay))), 0, CIDEAGREEN);
				}
			}
		pop();
    }
  ...
  placeTitleBold("Several mimics of a burst of up-and-down motions: imitate the source after a distance-dependent delay");  
}
