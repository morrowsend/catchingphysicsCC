// WvMultiMimicryDelaySawtoothUpAndDownPID
...
//            ready to go
        ...
        case 1:
//            running
			placeWords("pause",28,height-32);            runtime++;
            break;
        case 2:
//            paused
    ...
    if (runtime!=0){
		push();
			translate(130,200);
			showDisplacement(theamplitude*((runtime*thefrequency)-floor(runtime*thefrequency)),0, CIDEABLUE);
			translate(mimicoffset/2, 130);
			showDuration(thedelay, maxDelay);
			translate(-mimicoffset/2, -130);
			if (mimicbutton.buttonisChecked){
				for (i = 1; i < numbermimics; i++) {
					...
					showDisplacement(theamplitude*(((runtime-i*thedelay)*thefrequency)-floor((runtime-i*thedelay)*thefrequency)),0, CIDEAGREEN);
				}
			}
		pop();
    }
