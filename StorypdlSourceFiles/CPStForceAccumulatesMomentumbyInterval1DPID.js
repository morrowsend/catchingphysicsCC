// {CPStForceAccumulatesMomentumbyInterval1DPID}{900}{650}
...
// 	accumulate
	...
	push();
		...
		for (let i = 0; i<pqfluxions.length; i++){
			showForce(pqfluxions[i].mag(), degrees(-pqfluxions[i].heading()+PI/2), CIDEABLUE);
			translate(xstepdisplay, 0);
				}
    pop();
    ...
    push();
		...
		for (let i = 0; i<pqfluents.length; i++){
				showMomentum(pqfluents[i].mag(), degrees(-pqfluents[i].heading()+PI/2), CIDEAGREEN);
			translate(xstepdisplay, 0);
			}
    pop();


	placeWords("set\ninitial\nmomentum", xlocfluent1+xstepdisplay*(fluxions.length)+95, 188);
    ...
    placeTitleBold("Force accumulates momentum, interval by interval");
}
