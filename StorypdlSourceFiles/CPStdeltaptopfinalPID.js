...
function draw() {
    ...
    push();
		...
		showForce(accumulateacceleration.mag(), degrees(accumulateacceleration.heading())+90, CIDEAGREY);
	pop();
	push();
	...
	push();
		...
		showMomentum(accumulatedvelocity.mag(), degrees(accumulatedvelocity.heading())+90, CIDEAGREEN);
	pop();
	placeWords("set force", 30, 100);
	placeWords("set duration", 30, 280);

 //bottom pane display
	push();
		translate(250, 530);
		showMomentum(initialvelocity.mag(), degrees(initialvelocity.heading())+90, CIDEARED);
		push();
			translate(80, 0);
		...
		if (abs(deltavelocitylocationoffset.y-initialvelocity.y*PXSCALE)<5 && (abs(deltavelocitylocationoffset.x-initialvelocity.x*PXSCALE)-80)<5 && deltavelocitylocationoffset.y!=0 && deltavelocitylocationoffset.x!=0){
				showMomentum(finalvelocity.mag(), degrees(finalvelocity.heading())+90, CIDEABLUE);
		}
	pop();
	placeWords("set\ninitial\nmomentum", 120, 510);
	placeWords("place\ntail to tip", 570, 510);

  placeTitleBold("Calculating an accumulation in momentum, then a resultant momentum");
}
