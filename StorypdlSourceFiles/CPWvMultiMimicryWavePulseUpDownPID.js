// WvMultiMimicryWavePulseUpDownPID 950 400
...
//            paused
    ...
    if (runtime!=0){
		push();
			translate(130,200);
				showDisplacement(theamplitude*exp(-0.25*pow(thefrequency*runtime,2)),0, CIDEABLUE);
			translate(mimicoffset/2, 130);
			showDuration(thedelay, maxDelay);
			translate(-mimicoffset/2, -130);
			if (mimicbutton.buttonisChecked){
				for (i = 1; i < numbermimics; i++) {
					...
					showDisplacement(theamplitude*exp(-0.25*pow(thefrequency*(runtime-i*thedelay),2)), 0, CIDEAGREEN);
				}
			}
		pop();
    }
  ...
  placeTitleBold("Several mimics for an up-and-down motion: imitate the source after a distance-dependent delay");  
}
